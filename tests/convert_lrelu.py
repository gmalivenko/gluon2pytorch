...
class LReLUTest(mx.gluon.nn.HybridSequential):
    def __init__(self):
        super(LReLUTest, self).__init__()
        from mxnet.gluon import nn
        with self.name_scope():
            self.conv1 = nn.Conv2D(3, 32)
            self.leaky = nn.LeakyReLU(alpha=0.1)

    def hybrid_forward(self, F, x):
        x = F.relu(self.leaky(self.conv1(x)))
        return x
...
if __name__ == '__main__':
    ...
    pytorch_model = gluon2pytorch(net, [(1, 3, 224, 224)], dst_dir=None, pytorch_module_name='LReLUTest')
